...
p5.js 映像生成を受け継ぐ部分で、9種の手続き生成が用意されている:

`晴 CLEAR`（星の瞬き）/ `雨 RAIN` / `雪 SNOW`（揺れる雪片）/ `霧 FOG`（漂う霧）/
`雷雨 STORM`（雨＋稲妻の閃光）/ `極光 AURORA`（流れる光のリボン）/ `桜 SAKURA`（舞う花びら）/
`流星 METEOR`（星空＋流れ星）/ `砂嵐 SAND`（横殴りの砂塵）

- 左上のタグをクリック、または `W` で切り替え。`?weather=aurora` のように URL で固定も可能。
- `city.json` に `"weather": "storm"` を書けば既定の天候をピン留めできる。

**連動（記事・深度に反応する空）**

- **最新記事の気分 → 空**: 何もピン留めしなければ、天候は **最新記事のアーキタイプ**の
  affinity から選ばれる（廃墟→霧/雨/雷、歓楽街→極光/桜/晴、聖域→雪/霧/極光 …）。
  affinity 内でランダムなので、都市の空は「いま最後に書いた記事の気分」を映しつつ毎回変わる。
- **潜行の深さ → 霧**: アーカイブを深く潜るほど深度霧が濃くなり、古い地層ほど霞んで
  「忘れられていく」。表層の天候とは独立してかかる。

新しい天候を足すのも建物フォームと同じレジストリ方式:
`WEATHER_TYPES` / `WEATHER_LABEL` / `initWeather` の分岐 / `drawWeatherBack|Front` を
`map.js` に足し、必要なら `ARCH_WX` に affinity を書くだけ。
